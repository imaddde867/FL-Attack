...
# Colors (light theme with professional contrast)
BG_COLOR = (245, 247, 250)  # Light gray-white
CARD_BG = (255, 255, 255)   # Pure white cards
CARD_BORDER = (200, 210, 225)
HEADER_BG = (30, 60, 110)  # Dark blue header for contrast
TEXT_WHITE = (255, 255, 255)  # White for header text
TEXT_DARK = (30, 35, 45)  # Dark text for body
TEXT_LIGHT = (60, 70, 85)  # Medium dark text
TEXT_MUTED = (120, 130, 150)
ACCENT_BLUE = (40, 100, 200)   # Deeper blue for light bg
ACCENT_GREEN = (30, 150, 80)  # Deeper green
ACCENT_ORANGE = (220, 130, 20)  # Deeper orange
ACCENT_RED = (200, 50, 50)
ACCENT_PURPLE = (130, 60, 180)  # Deeper purple
ACCENT_GOLD = (180, 140, 0)  # Darker gold for light bg

# Column header colors - professional on light
COL_COLORS = [
    (50, 120, 220),   # Blue - Baseline
    (230, 140, 30),   # Orange - DP
    (40, 160, 90),    # Green - HE
    (140, 70, 190),   # Purple - DP+HE
]
...
class PosterComposer:
    ...
    def draw_header(self):
        ...
        self.draw.text((OUTER_MARGIN + 25, 120), subtitle, fill=TEXT_WHITE, font=self.font_subheader)
        ...
        draw_rounded_rect(self.draw, (result_box_x, 40, CANVAS_WIDTH - OUTER_MARGIN, 100),
                          radius=10, fill=(240, 230, 200), outline=ACCENT_GOLD, width=3)
    ...
    def _render_dp_column(self, area: Tuple[int, int, int, int], w: int, h: int):
        ...
        for label, setting in eps_variants:
            ...
            mx = x + img.width + 12
            my = current_y + 5
            self.draw.text((mx, my), f"LPIPS: {lpips:.3f}", fill=TEXT_DARK, font=self.font_caption)
            my += line_h
            self.draw.text((mx, my), f"SSIM: {ssim:.3f}", fill=TEXT_DARK, font=self.font_caption)
            my += line_h
            self.draw.text((mx, my), f"PSNR: {psnr:.1f} dB", fill=TEXT_DARK, font=self.font_caption)
            my += line_h + 10
    ...
    def _render_he_column(self, area: Tuple[int, int, int, int], w: int, h: int):
        ...
        self.draw.text((x, current_y), f"LPIPS: {lpips:.3f}   SSIM: {ssim:.3f}", fill=TEXT_DARK, font=self.font_caption)
        current_y += line_h
        self.draw.text((x, current_y), f"PSNR: {psnr:.1f} dB", fill=TEXT_DARK, font=self.font_caption)
        current_y += line_h
        ...
        draw_rounded_rect(self.draw, (x, current_y, x + w, current_y + delta_box_h),
                          radius=10, fill=(230, 245, 235), outline=ACCENT_GREEN, width=3)

        dy = current_y + 25
        self.draw.text((x + 25, dy), "Defense Impact:", fill=ACCENT_GREEN, font=self.font_body)
        dy += 45
        self.draw.text((x + 25, dy), f"dPSNR = {d_psnr:+.1f} dB", fill=TEXT_DARK, font=self.font_caption)
        dy += 35
        self.draw.text((x + 25, dy), f"dLPIPS = {d_lpips:+.3f}", fill=TEXT_DARK, font=self.font_caption)
    ...
    def _render_dphe_column(self, area: Tuple[int, int, int, int], w: int, h: int):
        ...
        delta_box_h = 150
        draw_rounded_rect(self.draw, (x, current_y, x + w, current_y + delta_box_h),
                          radius=10, fill=(240, 235, 250), outline=ACCENT_PURPLE, width=3)

        dy = current_y + 25
        self.draw.text((x + 25, dy), "IMPACT:", fill=ACCENT_PURPLE, font=self.font_body)
        dy += 45
    ...
    def _draw_metrics_panel(self, x: int, y: int, w: int, agg: Dict, title: str,
        ...
        draw_rounded_rect(self.draw, (x, y, x + w, y + panel_h),
                          radius=8, fill=(235, 240, 248), outline=CARD_BORDER, width=1)

        self.draw.text((x + 15, y + 12), title, fill=ACCENT_BLUE, font=self.font_caption)
        ...
        for metric in ['PSNR', 'SSIM', 'LPIPS']:
            if metric in agg and ly + line_h < y + panel_h - 10:
                ...
                self.draw.text((x + 15, ly), text, fill=TEXT_DARK, font=self.font_small)
                ly += line_h
    ...
    def draw_bottom_band(self):
        ...
        for i, chart in enumerate(charts):
            ...
            draw_rounded_rect(self.draw, (card_x, content_y, card_x + single_chart_w, content_y + chart_h),
                              radius=8, fill=(250, 252, 255), outline=CARD_BORDER, width=1)
        ...
        draw_rounded_rect(self.draw, (right_x, content_y, right_x + right_card_w, content_y + right_card_h),
                          radius=10, fill=(250, 252, 255), outline=ACCENT_BLUE, width=2)
        ...
        self.draw.text((text_x, text_y), "BEST ATTACK SETTINGS", fill=ACCENT_ORANGE, font=self.font_body)
        text_y += 45
        ...
        for idx, row in agg_by_type.head(3).iterrows():
            ...
            self.draw.text((text_x, text_y), line, fill=TEXT_DARK, font=self.font_caption)
            text_y += 32
...
def main():
    ...
    # Save PNG
    print("\n[3/4] Saving poster...")
    png_path = REPORT_DIR / 'poster_4k_light.png'
    poster.save(png_path, 'PNG', optimize=True)
    print(f"      Saved: {png_path}")

    # Try to save PDF
    pdf_path = REPORT_DIR / 'poster_4k_light.pdf'
    try:
